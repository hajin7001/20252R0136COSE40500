...
class nn_convolutional_layer:
    ...
    def forward(self, x):
        ...
        x_win = view_as_windows(x, (1, 1, f_height, f_width)) # x_win: (b_size, ch_size, out_height, out_width, 1, 1, f_height, f_width) 
        
        b_size, ch_size, out_height, out_width, _, _, f_height, f_width = x_win.shape

        # x_win을 (b_size, out_height*out_width, ch_size*f_height*f_width) 형태로 펴기
        x_col = x_win.reshape(b_size, ch_size, out_height*out_width, f_height*f_width)
        x_col = x_col.permute(0, 2, 1, 3).reshape(b_size, out_height*out_width, ch_size*f_height*f_width)

        # W를 (out_ch_size, ch_size*f_height*f_width) 로 펴기
        W_col = self.W.reshape(out_ch_size, ch_size*f_height*f_width)
        
        out = torch.matmul(x_col, W_col.T) # (b_size, out_height*out_weight, out_ch_size)
        out = out.transpose(1, 2) # (b_size, out_ch_size, out_height*out_weight)
        out = out.reshape(b_size, out_ch_size, out_height, out_width)
...
class nn_max_pooling_layer:
    ...
    def forward(self, x):
        b_size, _, _, _ = x.shape
        x_win = view_as_windows(x, (1, 1, self.pool_size, self.pool_size), step = (1, 1, self.stride, self.stride)) 
        # x_win: (b_size, ch_size, out_height, out_width, 1, 1, self.pool_size, self.pool_size) 
        
        b_size, ch_size, out_height, out_width, _, _, _, _ = x_win.shape
        x_win = x_win.reshape(b_size, ch_size, out_height, out_width, -1) 
        out = x_win.max(dim=-1).values
